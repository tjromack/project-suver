...
> rate-limiting · Secure cookies · **daily usage quotas + free/pro tiers** so public exposure can't run up the API
> bill) with a `Dockerfile`, a `DESIGN-PARTNER-KIT.md` onboarding runbook, and a `LAUNCH-READINESS.md` path-to-paid.
> Every tool has a **✨ Try an example** button — a real cited result in one click. See `CLIENT-ADAPTATION.md` for
> adopting it to a specific org. 189 tests (stub-backed,
